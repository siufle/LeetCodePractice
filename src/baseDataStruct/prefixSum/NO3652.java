...
public class NO3652 {
    public static void main(String[] args) {
        ...
        int[] prices = {7,1,11,22,29};
        int[] strategy = {0,-1,-1,-1,-1};
        int k = 4;
    ...
    static class Solution {
        public long maxProfit(int[] prices, int[] strategy, int k) {
            ...
            prefixSum[0] = 0;
            for (int i = 1; i <= n; i++) {
                prefixSum[i] = prefixSum[i - 1] + (long) prices[i - 1] * strategy[i - 1];
            }
            long temp = 0;
            for(int i = k/2; i < k; i++){
                temp += prices[i];
            }
            long res = Math.max(prefixSum[n] - prefixSum[k] + temp, prefixSum[n]);
            for(int i = 1; i < n - k + 1; i++){
                temp = temp - prices[i + k/2 - 1] + prices[i + k - 1];
                res = Math.max(res, prefixSum[n] - prefixSum[i + k]  + prefixSum[i] + temp);
            }
            return res;
        }
    }
